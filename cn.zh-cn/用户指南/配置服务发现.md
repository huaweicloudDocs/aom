...
-   **自动发现**
    ...
    您的主机[安装ICAgent](安装ICAgent.md)后，ICAgent会根据[内置服务发现规则](#section938317591962)发现主机上的服务，并呈现在“服务列表”界面。

-   **手动配置**

    您可在“服务发现”界面添加一条自定义的服务发现规则，并应用至已[安装ICAgent](安装ICAgent.md)的主机上，ICAgent会根据您配置的服务发现规则发现主机上的服务，并呈现在“服务列表”界面。
...
<tr id="row2581615122710"><td class="cellrowborder" valign="top" width="50%" headers="mcps1.2.3.1.1 "><p id="p3277356193717"><a name="p3277356193717"></a><a name="p3277356193717"></a>如果进程的“CMD”列的值以“/user/bin/”、“/user/sbin/”或“/sbin/”开头，则该类进程会被过滤掉，不会被AOM发现。</p>
</td>
<td class="cellrowborder" valign="top" width="50%" headers="mcps1.2.3.1.2 "><p id="p2033985722711"><a name="p2033985722711"></a><a name="p2033985722711"></a>例如，上面回显信息中“PID”为“1140”、“1144”、“1148”的进程，因为其“CMD”列的值分别以“/user/bin/”、“/user/sbin/”、“/sbin/”开头，所以这3个进程均不会被AOM发现。</p>
</td>
</tr>
</tbody>
</table>
...
3.  单击“添加自定义服务发现规则”，配置服务发现规则。
4.  选择预探测主机。

    1.  自定义一个规则规则名称（例如，ruletest）。
    2.  选择一个典型的主机（例如，hhhhhh-27465），用于在服务发现规则配置过程中预验证规则的有效性，最终在哪些主机上执行本规则，将会在[7](#li1434613512472)进行配置。完成后单击“下一步”。
...
6.  设置应用名称、服务名称及日志路径。
    ...
    3.  选择是否采集进程的日志。
        ...
        默认开启此功能，如果您不需要可自行关闭，并且在服务名称预览表格中不会出现关于日志的列表。
...
7.  <a name="li1434613512472"></a>设置优先级和探测范围。
...
9.  等待大约两分钟后，您可在左侧导航栏中选择“应用监控 \> 服务列表”，在集群下拉列表框中选择主机（例如，hhhhhh-27465），找到已被监控的服务（例如，/openvswitch/）。
