# 日志结构化<a name="ZH-CN_TOPIC_0152502858"></a>

该功能仅对华南-广州区域生效。

数据可分为结构化数据和非结构化数据。结构化数据：能够用数字或统一的数据模型加以描述的数据，具有严格的长度和格式。非结构化数据：数据结构不规则或不完整，没有预定义的数据模型，不方便用数据库二维逻辑表来表现的数据。

日志结构化是指以日志桶为单位，通过提取规则将日志桶中的日志进行结构化，提取出有固定格式或相似度较高的日志，过滤掉不相关的日志。
...
通过对日志桶添加提取规则，将日志桶中的原始日志按一定的规律进行提取，并将提取后的日志整合到一起，以便进行SQL查询与分析。

下面以Tomcat服务器的原始日志为例进行结构化：

1.  登录[应用运维管理](https://console.huaweicloud.com/aom/#/aom/ams/summary)。
2.  在左侧导航栏中选择“日志管理 \> 日志结构化”。
3.  结构化是以日志桶为单位，请先创建一个日志桶，操作详见[添加日志桶](添加日志桶.md)。若您已创建过日志桶，请跳过该步骤。

    日志桶创建完成后，日志桶信息将自动同步至“日志结构化”界面，如下图所示。

    **图 1**  日志桶<a name="fig178010725911"></a>  
    ![](figures/日志桶.png "日志桶")

4.  在“日志结构化”界面，选择目标日志桶，单击“添加提取规则”。
5.  选择示例日志：通过时间和关键词对日志桶中的原始日志进行查询与分析，初步找出可结构化的日志内容，然后选择一条比较典型的日志作为示例日志。

    如下图所示，可看到日志桶中的日志的内容比较规整，初步找出了待结构化的字段：时间、线程、IP、方法、url、返回码、时延，在日志列表中单击第一条日志，即把第一行内容作为了示例日志，然后单击“下一步”。

    **图 2**  选择示例日志<a name="fig8110132813195"></a>  
...
6.  选择提取方法。
    ...
    **图 3**  选择提取方法<a name="fig81896481110"></a>  
    ![](figures/选择提取方法.png "选择提取方法")

7.  选择字段。

    在示例日志中，选中待结构化的日志内容，在弹出的对话框中为选中内容设置一个名称。

    如下图所示，分别对时间、线程、IP、方法、url、返回码、时延内容设置名称，其中使用time表示时间，thread表示线程，IP表示IP地址，method表示方法，code表示返回码，delay表示时延。依次提取出time、thread、IP、method、url、code、delay字段。

    若某条原始日志不符合提取规则，则无法进行提取，只显示原始日志。

    **图 4**  选择字段<a name="fig8447155121410"></a>  
    ![](figures/选择字段.png "选择字段")

8.  单击“下一步”，确认无误后单击“确认”。完成日志结构化。

    这样已对Tomcat服务器的原始日志进行了结构化，结构化后的日志数据可理解为数据库中的二维表，接下来就可以使用SQL语句对time、thread、IP、method、url、code、delay字段进行查询与分析。


## SQL查询与分析<a name="section313011674316"></a>

对原始日志结构化后，等待1\~2分钟左右即可基于结构化的日志进行SQL查询与分析。

下面以结构化后的Tomcat服务器的日志为例进行SQL查询与分析，定位出Tomcat服务器运行异常的原因。

1.  在左侧导航栏中选择“日志管理 \> 日志桶”。
2.  SQL查询也是以日志桶为单位，请在“桶日志”页签选择目标日志桶，然后单击“结构化日志”。

    **图 5**  结构化后的日志<a name="fig193619120351"></a>  
    ![](figures/结构化后的日志.png "结构化后的日志")

3.  SQL查询与分析：设置时间范围和查询条件。查询条件请输入SQL语句，目前AOM支持的SQL语句详见[SQL查询语法](#section128657218189)。

    例如，需查询近6小时内HTTP返回码大于等于500的请求数、近6小时内HTTP请求的平均时延，则可执行如下操作：

    在右上角的下拉列表框选择“近6小时”，在搜索框中分别输入SQL语句：select count\(\*\) where code \>= 500、select avg\(delay\)，如[图6](#fig1559711773614)所示 。
    ...
    >-   如果SQL语句返回的是单个数值，例如：select count\(\*\) where code \>= 500、select avg\(delay\)，则支持创建统计规则；如果SQL语句返回的是多个值，例如：select count\(\*\) group by IP、select \*，则不支持创建统计规则。  

    **图 6**  使用SQL语句进行查询<a name="fig1559711773614"></a>  
