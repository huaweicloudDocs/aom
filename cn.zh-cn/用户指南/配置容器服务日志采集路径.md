...
-   **集群方式**：在容器服务创建完成后，在AOM界面以集群为单位同时对集群下的所有容器的日志采集路径进行配置。如果集群下容器服务的日志采集路径是相同的，推荐使用此方式。
    ...
    该配置方式分为单个配置和批量配置。单个配置一次只能配置一条日志采集路径，批量配置一次可配置多条日志采集路径。批量配置使用JSON格式，若您对JSON没有初步的了解，推荐您使用单个配置方式。操作详见[集群方式-单个配置](#section179891631134913)和[集群方式-批量配置](#section1381110186)。

-   **单容器服务方式**：在CCE、ServiceStage、CCI界面创建容器服务时，通过添加日志策略单独对指定的单个容器服务的日志采集路径进行配置。操作详见[单容器服务方式](#section20934195113)。

## 集群方式-单个配置<a name="section179891631134913"></a>
...
3.  配置方式选择“单个配置”，参考[表1](#table567955913453)配置相关参数，如下图所示。
    ...
    **表 1**  参数说明
    ...
    <td class="cellrowborder" valign="top" headers="mcps1.2.4.1.2 "><p id="p967813598458"><a name="p967813598458"></a><a name="p967813598458"></a>用来唯一标识一次路径配置操作。</p>
    <p id="p1867817596454"><a name="p1867817596454"></a><a name="p1867817596454"></a>长度为1～64位字符，不能包含"$@#%&amp;'+;&lt;=&gt;?\特殊字符，不能重复，不能为空，配置成功后不可修改。</p>
    </td>
    </tr>
    ...
    <p id="p10678185944512"><a name="p10678185944512"></a><a name="p10678185944512"></a>目录必须以/开头，至少为一级目录，最多支持采集已配目录下的三级子目录。目录必须是实际存在的，否则配置无意义。</p>
    ...
    <a name="ul1667855916458"></a><a name="ul1667855916458"></a><ul id="ul1667855916458"><li>配置：文件名称必须以.log、.out、.trace结尾。文件名称支持配置为 *.log、*.trace或*.out。</li></ul>
    </td>
    </tr>
...
## 集群方式-批量配置<a name="section1381110186"></a>
...
3.  配置方式选择“批量配置”，通过JSON格式的配置样例批量配置多个规则。
    ...
    **表 2**  参数说明
    ...
    <td class="cellrowborder" valign="top" width="70%" headers="mcps1.2.4.1.3 "><p id="p5509180111411"><a name="p5509180111411"></a><a name="p5509180111411"></a>用来唯一标识一次路径配置操作。</p>
    ...
    <p id="p85112011419"><a name="p85112011419"></a><a name="p85112011419"></a>目录必须以/开头，至少为一级目录，最多支持采集已配目录下的三级子目录。目录必须是实际存在的，否则配置无意义。</p>
    ...
    <a name="ul7511140181410"></a><a name="ul7511140181410"></a><ul id="ul7511140181410"><li>配置：文件名称必须以.log、.out、.trace结尾。文件名称支持配置为  *.log、*.trace或*.out。</li></ul>
    </td>
    </tr>
...
4.  （可选）单击“格式检查”，对JSON格式进行检查，格式无误后单击“确认”，完成配置。
    ...
    AOM支持对JSON格式进行一键检查，当输入JSON数据较长，一时无法判断是否有有多或少输入符号导致AOM不能解析时，可单击“格式检查”，快速检查已配置的内容是否满足JSON格式要求。如果不满足JSON格式要求，可参考配置样例进行检查并修改。
...
-   **查看容器服务日志并进行分析**
    ...
    **图 5**  查看容器服务日志并进行分析<a name="fig12122108161919"></a>  
